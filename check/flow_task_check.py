...
def flow_task_exe_res_dto_check(dto:FlowTaskExeResDTO):
    ...
    if dto.status == False and str_tools.str_is_empty(dto.error_msg):
        raise EasyRpaException("error msg is empty",EasyRpaExceptionCodeEnum.DATA_NULL.value[1],None,dto)
    if dto.status == True and str_tools.str_is_empty(dto.result):
        raise EasyRpaException("result is empty",EasyRpaExceptionCodeEnum.DATA_NULL.value[1],None,dto)
    return True