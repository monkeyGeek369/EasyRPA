...
def flow_task_subscribe_dto_check(dto:FlowTaskSubscribeDTO)->bool:
    if not dto:
        raise EasyRpaException("flow task subscribe obj is empty",EasyRpaExceptionCodeEnum.DATA_NULL.code,None,dto)
    ...
    if number_tool.num_is_empty(dto.flow_configuration_id):
        raise EasyRpaException("flow configuration id is empty",EasyRpaExceptionCodeEnum.DATA_NULL.code,None,dto)
    if str_tools.str_is_empty(dto.biz_no):
        raise EasyRpaException("biz no is empty",EasyRpaExceptionCodeEnum.DATA_NULL.code,None,dto)
    ...
    if str_tools.str_is_empty(dto.request_standard_message):
        raise EasyRpaException("request standard message is empty",EasyRpaExceptionCodeEnum.DATA_NULL.code,None,dto)
    return True
